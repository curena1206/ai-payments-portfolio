...
const USE_CASES = [
  {
    id:'B2B',
    name:'B2B Payment Flow Disbursements',
    icon:'⬡',
    tam:32000,
    penetration:4.2,
  ...
  },
  {
    ...
    status:'Ahead',
    insight:'Stablecoin payout pilot with 3 platforms launched Q3. Creator economy driving outsized growth in US→PH and SG→PH flows.',
    flows:['US→PH','SG→PH','US→MX'],
  },
  {
    id:'P2P',
    ...
    status:'On Track',
    insight:'Largest volume flow. Bank channel dominates. Digital app share growing — potential fee compression risk if direct-to-consumer rails gain traction.',
    flows:['US→MX','US→PH','US→IN','US→NG'],
  },
  {
    id:'GOV',
    ...
    status:'Behind',
    insight:'Regulatory complexity slowing onboarding in NG and BR flows. Two government contracts in final procurement stage. Expected to close H1.',
    flows:['US→NG','US→BR','EU→NG'],
  },
];

// ── GTM PIPELINE DATA ─────────────────────────────────────────────────────────
const PIPELINE = [
  { id:'P001', client:'Banco Santander Mexico',  type:'Bank',    useCase:'B2B',  flow:'US→MX', stage:'Contracting', vol:850,  rev:5.5,  days:18, risk:'Low' },
  { id:'P002', client:'GCash / Mynt',            type:'Fintech', useCase:'GIG',  flow:'US→PH', stage:'Contracting', vol:620,  rev:4.6,  days:24, risk:'Low' },
  { id:'P003', client:'HDFC Bank',               type:'Bank',    useCase:'B2B',  flow:'US→IN', stage:'Solutioning', vol:1200, rev:7.2,  days:45, risk:'Med' },
  { id:'P004', client:'Nubank Brazil',           type:'Fintech', useCase:'P2P',  flow:'US→BR', stage:'Solutioning', vol:480,  rev:3.3,  days:38, risk:'Med' },
  { id:'P005', client:'Access Bank Nigeria',     type:'Bank',    useCase:'GOV',  flow:'US→NG', stage:'Qualified',   vol:320,  rev:3.4,  days:62, risk:'High' },
  { id:'P006', client:'Grab Financial',          type:'Platform',useCase:'GIG',  flow:'SG→PH', stage:'Contracting', vol:410,  rev:3.3,  days:12, risk:'Low' },
  { id:'P007', client:'Standard Chartered UK',  type:'Bank',    useCase:'B2B',  flow:'UK→IN', stage:'Live',        vol:890,  rev:7.0,  days:0,  risk:'None' },
  { id:'P008', client:'Wise Platform',           type:'Fintech', useCase:'P2P',  flow:'EU→NG', stage:'Qualified',   vol:210,  rev:2.3,  days:55, risk:'High' },
  { id:'P009', client:'Remitly',                 type:'Fintech', useCase:'P2P',  flow:'US→PH', stage:'Live',        vol:740,  rev:5.5,  days:0,  risk:'None' },
  { id:'P010', client:'Corpay Payment Flow',     type:'Corporate',useCase:'B2B', flow:'US→MX', stage:'Solutioning', vol:560,  rev:3.9,  days:41, risk:'Med' },
];
...
export default function Model06() {
  ...
  const totalVol = FLOWS.reduce((s,c) => s+c.vol, 0);
  const totalRev = FLOWS.reduce((s,c) => s+c.rev, 0);
  const avgGrowth = (FLOWS.reduce((s,c) => s+c.growth, 0) / FLOWS.length).toFixed(1);
  const growFlows = FLOWS.filter(c => c.status === 'Grow').length;
          ...
          <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:24 }}>
            <div>
              ...
              <p style={{ fontFamily:T.sans, fontSize:14, color:T.textMid, maxWidth:520, lineHeight:1.65 }}>
                Evaluates the bank's participation economics across push payment networks — assessing whether volume, pricing, and flow coverage are generating an adequate return on infrastructure investment.
              </p>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, minWidth:480 }}>
              ...
              <KPI label="Grow Flows" value={`${growFlows}/8`} sub="On expansion track" accent={T.green} />
            </div>
          </div>
          ...
          <div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:24 }}>
              ...
              <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:8, overflow:'hidden' }}>
                <div style={{ padding:'14px 20px', borderBottom:`1px solid ${T.border}`, fontFamily:T.mono, fontSize:11, color:T.gold, letterSpacing:'0.08em', textTransform:'uppercase' }}>
                  Flow Rankings — Net Revenue
                </div>
                {[...FLOWS].sort((a,b) => b.net - a.net).map((c,i) => (
                  <div key={c.id} className="m06-cor"
                    onClick={() => setSelectedFlow(c)}
                    style={{ padding:'12px 20px', borderBottom:`1px solid ${T.border}`, display:'grid', gridTemplateColumns:'28px 1fr 60px 70px 80px', gap:12, alignItems:'center', background: selectedFlow.id === c.id ? T.cardHov : 'transparent', transition:'background 0.15s' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                <div style={{ background:T.card, border:`1px solid ${T.gold}30`, borderRadius:8, padding:20 }}>
                  <div style={{ fontFamily:T.mono, fontSize:10, color:T.gold, letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:12 }}>
                    {selectedFlow.name} — Deep Dive
                  </div>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12, marginBottom:16 }}>
                    {[
                      { l:'Monthly Volume', v:`${fmtK(selectedFlow.vol)}`, u:'transactions' },
                      { l:'Revenue / Txn', v:`$${selectedFlow.rpm.toFixed(2)}`, u:'gross' },
                      { l:'Net / Txn', v:`$${selectedFlow.net.toFixed(2)}`, u:'after costs' },
                      { l:'FX Spread', v:`$${selectedFlow.fx.toFixed(2)}`, u:'per txn' },
                      { l:'Settlement Cost', v:`$${selectedFlow.settle.toFixed(2)}`, u:'per txn' },
                      { l:'Growth vs Plan', v:`${pct(selectedFlow.growth - selectedFlow.plan)}`, u:'variance' },
                    ].map((m,i) => (
                  ...
                  <ResponsiveContainer width="100%" height={80}>
                    <AreaChart data={selectedFlow.trend.map((v,i) => ({ m:MONTHS[i], v }))}>
                      <defs>
                ...
                <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:8, padding:20 }}>
                  ...
                  <ResponsiveContainer width="100%" height={140}>
                    <BarChart data={[
                      { name:'Gross Fee', v:selectedFlow.rpm, fill:T.gold },
                      { name:'FX Spread', v:selectedFlow.fx, fill:T.teal },
                      { name:'Settle Cost', v:-selectedFlow.settle, fill:T.red },
                      { name:'Other Cost', v:-(selectedFlow.rpm - selectedFlow.fx - selectedFlow.net - selectedFlow.settle), fill:T.amber },
                      { name:'Net Rev', v:selectedFlow.net, fill:T.green },
            ...
            <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:8, padding:20 }}>
              <div style={{ fontFamily:T.mono, fontSize:10, color:T.gold, letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:16 }}>
                All Flows — Growth vs Plan
              </div>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={FLOWS.map(c => ({ name:c.id, actual:c.growth, plan:c.plan }))} barGap={4}>
        ...
        {tab === 'pricing' && (
          <div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
              {/* Net revenue per transaction by flow */}
              <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:8, padding:20 }}>
                <div style={{ fontFamily:T.mono, fontSize:10, color:T.gold, letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:16 }}>
                  Net Revenue / Transaction by Flow
                </div>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={[...FLOWS].sort((a,b) => b.net - a.net).map(c => ({ name:c.id, net:c.net, fx:c.fx, settle:c.settle }))} layout="vertical">
                    ...
                    <Bar dataKey="net" name="Net Rev" radius={[0,3,3,0]}>
                      {FLOWS.map((c,i) => <Cell key={i} fill={c.net >= 4.5 ? T.green : c.net >= 4.0 ? T.teal : T.amber} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
              ...
              <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:8, padding:20 }}>
                ...
                {[
                  ...
                  { scenario:'Volume +10% across all flows', impact:+8.7, type:'opp'  },
                  { scenario:'Scheme fee +$0.25/txn',            impact:+3.2, type:'opp'  },
                  { scenario:'New flow launch (US→CN)',       impact:+4.5, type:'opp'  },
                ...
                <table style={{ width:'100%', borderCollapse:'collapse' }}>
                  <thead>
                    <tr style={{ borderBottom:`1px solid ${T.border}` }}>
                      {['Flow','Scheme Fee','FX Model','Review Status','Exception','Last Review','Next Review'].map(h => (
            ...
            <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:8, padding:20 }}>
              ...
              <div style={{ overflowX:'auto' }}>
                <table style={{ width:'100%', borderCollapse:'collapse' }}>
                  <thead>
                    <tr style={{ borderBottom:`1px solid ${T.border}` }}>
                      {['Client','Type','Use Case','Flow','Stage','Vol (K/mo)','Rev ($M/mo)','Days in Stage','Risk'].map(h => (
                  ...
                  <tbody>
                    {PIPELINE.map((p,i) => (
                      <tr key={p.id} className="m06-pipe" style={{ borderBottom:`1px solid ${T.border}`, transition:'background 0.1s' }}>
                        ...
                        <td style={{ padding:'10px 12px', fontFamily:T.mono, fontSize:11, color:T.gold }}>{p.flow}</td>
